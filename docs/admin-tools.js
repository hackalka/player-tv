...
(function () {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            #tg-personal { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
            ...
            .tg-cat { border: none; background: transparent; color: var(--tg-text); opacity: 0.7; padding: 6px 10px; border-radius: 6px; cursor: pointer; font-size: 0.85rem; }
            ...
            .tg-msg { max-width: 65%; padding: 8px 12px; border-radius: 12px; background: var(--tg-bubble-in); color: var(--tg-bubble-text); align-self: flex-start; box-shadow: 0 1px 2px rgba(0,0,0,0.1); position: relative; }
            ...
            /* Panel Lateral de Edición de Info */
            ...
            <div class="modal-content tg-card theme-${state.theme}">
                ...
                <div class="tg-layout">
                    <aside class="tg-sidebar">
                        <header class="tg-side-head" style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <section class="tg-main">
                        <header class="tg-main-head" id="tg-main-head">
                            <div style="padding:18px; font-weight:600; opacity:0.7;">Selecciona una comunidad para gestionar</div>
                        </header>
                        <div style="display:flex; flex:1; overflow:hidden;">
                            <div style="display:flex; flex-direction:column; flex:1; height:100%;">
                                ...
                                <footer class="tg-compose" id="tg-compose" hidden>
                                    ...
                                    <button id="tg-send" style="background:transparent; border:none; color:var(--tg-active); font-weight:bold; cursor:pointer;">Enviar</button>
                                </footer>
                            </div>
                            <!-- Panel de Edición de Canales/Grupos -->
    ...
    async function open() { buildModal(); $('#tg-personal').hidden = false; await loadChats(); }
    ...
    async function selectChat(peer, title) {
        ...
        $('#tg-edit-panel').hidden = true; // Cerrado por defecto hasta pulsar en la tuerca

        if (state.forwardCache) { await executeCleanForward(peer); return; }
    ...
    // Despliega y sincroniza el panel de edición de metadatos del Canal/Grupo
    ...
    // Envía las modificaciones a tu backend/API para actualizar Telegram de forma inmediata
    async function saveChatSettings() {
        ...
        try {
            // Llama a la ruta de guardado/edición de tu repositorio de administración
    ...
    async function executeCleanForward(targetPeer) {
        const txt = state.forwardCache; state.forwardCache = null; $('#tg-forward-alert').hidden = true;
        try { await api(`/api/admin/group/${encodeURIComponent(targetPeer)}/message`, { method: 'POST', body: JSON.stringify({ text: txt }) }); selectChat(targetPeer, state.currentTitle); } catch {}
    }

    async function onSendText() {
        const ta = $('#tg-text'); const text = ta.value.trim(); if (!text) return;
        ta.disabled = true;
        try { await api(`/api/admin/group/${encodeURIComponent(state.currentPeer)}/message`, { method: 'POST', body: JSON.stringify({ text }) }); ta.value = ''; await loadMessages(); } catch {} finally { ta.disabled = false; ta.focus(); }
    }
